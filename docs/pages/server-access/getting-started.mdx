...
2. `auth-admin` will now see:
  ...
  - `auth-admin` is now logged iton the `auth.example.com` cluster and `auth` Node via Teleport SSH - no third-party tool required.
...
Now, `auth-admin` has the ability to SSH into other Nodes within the cluster, traverse the Linux file system, and execute command.
...
2. As a result, `auth-admin` can execute `tsh ssh root@env=example` since each Node has [label](../admin-guide.mdx#labeling-nodes-and-applications) by default. 
   ...
   - This is a convenient feature that allows for more advanced query. For example, suppose an IP address changes (but the label doesn't). An admin can quickly find the current Node with that label.


3. `auth-admin` can also execute commands on all Nodes that have share a label vastly simplifying repeated operations:
